...
from agent_system.litegui import LiteGUIRewardShaper
...
class TrajectoryCollector:
    def __init__(self, config, tokenizer: PreTrainedTokenizer, processor=None):
        ...
        self.litegui_reward = LiteGUIRewardShaper(config.env.get('litegui', None))
    ...
    def vanilla_multi_turn_loop(
        ...
        # LiteGUI-RL is an additive extension to ClawGUI's trajectory collection.
        # It shapes the episode returns consumed by EpisodeRewardManager, while
        # the policy loss, old/ref log-probs, clipping, KL and GRPO advantages
        # continue to be computed by the original verl trainer.
        if self.litegui_reward.enabled:
            episode_rewards, _ = self.litegui_reward.shape_batch(
