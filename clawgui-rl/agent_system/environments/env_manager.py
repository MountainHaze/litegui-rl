...
from agent_system.litegui import screen_change_score
...
class MobileWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def __init__(self, envs, projection_f, config):
        ...
        # LiteGUI-RL uses a cheap deterministic screenshot-change signal.  It is
        # recorded in infos here and combined with the final outcome centrally
        # by TrajectoryCollector; verl/GRPO remains unchanged.
        litegui_config = config.env.get('litegui', {})
        self._litegui_enabled = bool(litegui_config.get('enable', False))
        if self._litegui_enabled:
            print("[MobileWorld] LiteGUI state-change reward ENABLED (no external PRM)")
        ...
        if self._litegui_enabled:
            for i, info in enumerate(infos):
                change = screen_change_score(
                    prev_images[i] if prev_images else "",
                    image_obs[i] if image_obs else "",
                )
                info['litegui_state_change'] = float(change)
                # Used to detect repeated actions in the trajectory shaper.
                info['litegui_action_signature'] = " ".join(text_actions[i].split())
